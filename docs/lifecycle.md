...
There are two moments in a source's life: **acquisition** (`cganno download`, once) and
**annotation** (`cganno annotate`, per query). What happens at each depends on the source
`type`.

| source | `cganno download` | `cganno annotate` |
| --- | --- | --- |
...
`cganno download` handles all of these in one pass over the snapshot's sources
...
tabix index is ensured: a prebuilt `url_index` is downloaded if given, otherwise cganno
...
requires = ["unzip", "python3"]                             # host deps, preflighted
  ...
  run = [                                                    # shell steps; the last must write {output}
    ...
    "python3 {workdir}/convert_csv_to_tab.py {workdir}/*.csv | cganno bgzip -o {output} -s 1 -b 2 -e 2 -S 1",
  ]
```

`cganno download` runs the recipe once (cached; `--force` rebuilds). A build source is
...
`cganno download` acquires a tool source's container image and runs its one-time setup:
...
2. **Setup** — the `[[sources.setup]]` steps run **once** to install the tool's data into
   ...
   `.cganno-setup-done` marker), so it runs on the first download and is skipped
...
The last step must write `{output}` (default `<name>.<format>.gz`), which cganno then reads
...
mountpoints**, *independent of the host layout* — cganno binds the matching host dirs to
...
| --- | --- | --- |
| `{datadir}` | `/cganno/data` | the tool's persistent data dir |
| `{workdir}` | `/cganno/work` | the per-run scratch dir |
| `{ref}` | `/cganno/ref/<file>` | the reference FASTA's dir |
| `{input}` | `/cganno/in/<file>` | the input file's dir |
| `{output}` | `/cganno/work/<file>` | (written under the workdir) |
...
next to the source's `.toml`, or URLs). cganno stages each into the step workdir before
every run, so a step references one as `{workdir}/<name>` — no `PATH` or shebang reliance,
and it works in host *and* container steps (the workdir is bound at `/cganno/work`).
...
"unzip"]`). `cganno download` and `cganno annotate` check them with one lookup up front and
...
cganno ships hidden `bgzip` and `tabix` subcommands backed by its `hts` library:

- `cganno bgzip [-o FILE] [file]` — BGZF-compress a file or stdin. Adding a tabix
  preset/columns (`-p vcf|bed|gff`, or `-s`/`-b`/`-e`/`-S`/`-c`/`-0`) **with `-o`** also
  writes the index in one step.
- `cganno tabix [preset|cols] FILE` — write a tabix index for an existing `.gz`.

Call them as `cganno bgzip` / `cganno tabix` from your `run` steps (cganno is on `PATH` when
your recipe runs).
